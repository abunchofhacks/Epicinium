...
AIStoryStingray::Tile AIStoryStingray::makeTile(Cell index) const
{
	Tile tile;
...
AIStoryStingray::Ground AIStoryStingray::makeGround(Cell index) const
{
	Ground ground;
...
AIStoryStingray::Air AIStoryStingray::makeAir(Cell index) const
{
	Air air;
...
bool AIStoryStingray::cityOccupied(Cell index) const
{
...
int8_t AIStoryStingray::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIStoryStingray::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIStoryStingray::AIStoryStingray(const Player& player, const Difficulty& difficulty,
...
void AIStoryStingray::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
void AIStoryStingray::checkUnfinished()
{
...
std::string AIStoryStingray::ainame() const
{
	return "StoryStingray";
}

std::string AIStoryStingray::authors() const
{
	return "Tom van den Bosch";
}

std::string AIStoryStingray::displayname() const
{
	return "Big evil guy";
}

// this is the part where I start shuffling things around and changing things
void AIStoryStingray::process()
{
	determineState();
	...
	if (_myGunners.size() < 2) _secondPhase = true;	
	
	
	if (_secondPhase == false)
	{
	for (Ground& gunner : _myGunners)
	{
	...
	// Once you defeat 3/4 gunners on the bridge, phase 2 starts

	
	if (_secondPhase == true)
	{
	...
	// Create new tanks and gunners if power==max(power)
