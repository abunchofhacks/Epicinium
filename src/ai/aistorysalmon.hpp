...
class AIStorySalmon : public AICommander
{
public:
	AIStorySalmon(const Player& player, const Difficulty& difficulty,
		const std::string& rulesetname, char character);
	AIStorySalmon(const AIStorySalmon&) = delete;
	AIStorySalmon(AIStorySalmon&&) = delete;
	AIStorySalmon& operator=(const AIStorySalmon&) = delete;
	AIStorySalmon& operator=(AIStorySalmon&&) = delete;
	virtual ~AIStorySalmon() = default;
