...
AIStorySalmon::Tile AIStorySalmon::makeTile(Cell index) const
{
	Tile tile;
...
AIStorySalmon::Ground AIStorySalmon::makeGround(Cell index) const
{
	Ground ground;
...
bool AIStorySalmon::cityOccupied(Cell index) const
{
...
int8_t AIStorySalmon::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIStorySalmon::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIStorySalmon::AIStorySalmon(const Player& player, const Difficulty& difficulty,
...
void AIStorySalmon::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
void AIStorySalmon::checkUnfinished()
{
...
std::string AIStorySalmon::ainame() const
{
	return "StorySalmon";
}

std::string AIStorySalmon::authors() const
{
	return "Tom van den Bosch";
}

// this is the part where I start shuffling things around and changing things
void AIStorySalmon::process()
{
	determineState();
	...
	// first order each turn: Make a single rifleman in middle barracks. If that is occupied or destroyed, make one in top. If that is also occupied or captured, make one in bottom.
	
	int _barracksNumber = 0;
	
	for (Tile& barracks : _myBarracks)
	{
		_barracksNumber++;
		Cell location = _board.cell(barracks.descriptor.position);
		if (_myBarracks.size() == 3 && _barracksNumber == 2)
		{
		...
		else if ((_myBarracks.size() == 2 || (_myBarracks.size() == 3 && occupiedbarracks.steps(location) == 9)) && _barracksNumber == 1)
		{
				Order order(Order::Type::PRODUCE, barracks.descriptor,
					Descriptor::cell(barracks.descriptor.position),
					_riflemantype);
		...
		else if (_myBarracks.size() == 1 || (_myBarracks.size() == 2 && occupiedbarracks.steps(location) == 13))
		{
				Order order(Order::Type::PRODUCE, barracks.descriptor,
					Descriptor::cell(barracks.descriptor.position),
					_riflemantype);
	...
	// Second: make all militia and riflemen focus if possible.
	...
	// Third: Move all riflemen to closest enemy building, capture if on one!
	for (Ground& rifleman : _myRiflemen)
	{
