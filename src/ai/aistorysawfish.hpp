...
class AIStorySawfish : public AICommander
{
public:
	AIStorySawfish(const Player& player, const Difficulty& difficulty,
		const std::string& rulesetname, char character);
	AIStorySawfish(const AIStorySawfish&) = delete;
	AIStorySawfish(AIStorySawfish&&) = delete;
	AIStorySawfish& operator=(const AIStorySawfish&) = delete;
	AIStorySawfish& operator=(AIStorySawfish&&) = delete;
	virtual ~AIStorySawfish() = default;
