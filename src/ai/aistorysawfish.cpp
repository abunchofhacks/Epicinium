...
AIStorySawfish::Tile AIStorySawfish::makeTile(Cell index) const
{
	Tile tile;
...
AIStorySawfish::Ground AIStorySawfish::makeGround(Cell index) const
{
	Ground ground;
...
AIStorySawfish::Air AIStorySawfish::makeAir(Cell index) const
{
	Air air;
...
bool AIStorySawfish::cityOccupied(Cell index) const
{
...
int8_t AIStorySawfish::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIStorySawfish::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIStorySawfish::AIStorySawfish(const Player& player, const Difficulty& difficulty,
...
void AIStorySawfish::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
void AIStorySawfish::checkUnfinished()
{
...
std::string AIStorySawfish::ainame() const
{
	return "StorySawfish";
}

std::string AIStorySawfish::authors() const
{
	return "Tom van den Bosch";
}

std::string AIStorySawfish::displayname() const
{
	return "Big evil guy";
}

// this is the part where I start shuffling things around and changing things
void AIStorySawfish::process()
{
	determineState();
	...
	// Turn 1: move zeppelins towards enemy cities. Northern gunner and sapper move towards enemy city as well.
	if (_turnNumber == 1)
	{
...
// everyturn loop:
	...
	for (Ground& sapper : _mySappers)
	{
		...
		Cell destination = _board.cell(sapper.descriptor.position);
						...
						for (Cell target : _board.area(from, rangeMin, rangeMax))
						{
							...
							if (_board.tile(target).type == _citytype && bestScore < 20){
								bestCell = target;
								bestScore = 20;
							}
		...
		// start moving in spring; least chance of dying to frostbite.
		if (_turnNumber > 16 && AICommander::_season == Season::SPRING)
		{
	...
	// Lockdown/focus/shell if possible
...
for (Ground& gunner : _myGunners)
	{
		...
		else if (occupiedcities.steps(destination) < 5)
		{
		std::vector<Move> moves;
		Move current;
	...
	// If year >= 4 create tanks and gunners if power=max(power)
	// If year >= 6 create sapppers instead of gunners, as gunners die to weather because of their low movement.
	// If distance to allied unoccupied city is >1 force create units in barracks and industry independent of power (as long as power>0).

	for (Tile& barracks : _myBarracks)
	{
		if (barracks.unfinished.type != Order::Type::NONE) continue;
		
		if (alliedcities.steps(_board.cell(barracks.descriptor.position)) > 1)
		{
	...
	for (Tile& industry : _myIndustry)
	{
		...
		if (alliedcities.steps(_board.cell(industry.descriptor.position)) > 1)
		{
