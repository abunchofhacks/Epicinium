...
AIActingAlpaca::Tile AIActingAlpaca::makeTile(Cell index) const
{
	Tile tile;
...
AIActingAlpaca::Ground AIActingAlpaca::makeGround(Cell index) const
{
	Ground ground;
...
AIActingAlpaca::Air AIActingAlpaca::makeAir(Cell index) const
{
	Air air;
...
bool AIActingAlpaca::cityOccupied(Cell index) const
{
...
int8_t AIActingAlpaca::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIActingAlpaca::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIActingAlpaca::AIActingAlpaca(const Player& player, const Difficulty& difficulty,
...
void AIActingAlpaca::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
void AIActingAlpaca::checkUnfinished()
{
...
std::string AIActingAlpaca::ainame() const
{
	return "ActingAlpaca";
}

std::string AIActingAlpaca::authors() const
{
	return "Tom van den Bosch";
}

std::string AIActingAlpaca::displayname() const
{
	return "Big evil guy";
}

// this is the part where I start shuffling things around and changing things
void AIActingAlpaca::process()
{
	determineState();
	...
	if (_turnNumber > 16)
	{
		for (Ground& sapper : _mySappers)
		{
			...
			if (bestScore > 0)
			{
				...
				_options.emplace_back(Option{ order, 9999 });
			}
		}
	}
	else if (_enemyTowns.size() == 3)
	{
	...
	{
		// win condition: SHELL own city if enemy has 3 towns
		// bombard enemy city if turn > 16
		for (Ground& sapper : _mySappers)
		{
			//Cell destination = _board.cell(sapper.descriptor.position);
			Cell from = _board.cell(sapper.descriptor.position);
			...
			for (Cell target : _board.area(from, rangeMin, rangeMax))
			{
				...
				if (_board.ground(target).type == _militiatype) {
					bestCell = target;
					bestScore = 1;
				}
		...
		for (Ground& militia : _myMilitia)
		{
			...
			if (enemyunits.steps(destination) < 5)
			{
