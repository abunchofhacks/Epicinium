...
class AIStorySeabass : public AICommander
{
public:
	AIStorySeabass(const Player& player, const Difficulty& difficulty,
		const std::string& rulesetname, char character);
	AIStorySeabass(const AIStorySeabass&) = delete;
	AIStorySeabass(AIStorySeabass&&) = delete;
	AIStorySeabass& operator=(const AIStorySeabass&) = delete;
	AIStorySeabass& operator=(AIStorySeabass&&) = delete;
	virtual ~AIStorySeabass() = default;
