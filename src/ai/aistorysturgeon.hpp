...
class AIStorySturgeon : public AICommander
{
public:
	AIStorySturgeon(const Player& player, const Difficulty& difficulty,
		const std::string& rulesetname, char character);
	AIStorySturgeon(const AIStorySturgeon&) = delete;
	AIStorySturgeon(AIStorySturgeon&&) = delete;
	AIStorySturgeon& operator=(const AIStorySturgeon&) = delete;
	AIStorySturgeon& operator=(AIStorySturgeon&&) = delete;
	virtual ~AIStorySturgeon() = default;
