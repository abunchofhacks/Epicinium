...
AIStorySeabass::Tile AIStorySeabass::makeTile(Cell index) const
{
	Tile tile;
...
AIStorySeabass::Ground AIStorySeabass::makeGround(Cell index) const
{
	Ground ground;
...
bool AIStorySeabass::cityOccupied(Cell index) const
{
...
int8_t AIStorySeabass::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIStorySeabass::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIStorySeabass::AIStorySeabass(const Player& player, const Difficulty& difficulty,
...
void AIStorySeabass::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
void AIStorySeabass::checkUnfinished()
{
...
std::string AIStorySeabass::ainame() const
{
	return "StorySeabass";
}

std::string AIStorySeabass::authors() const
{
	return "Tom van den Bosch";
}

std::string AIStorySeabass::displayname() const
{
	return "Big evil guy";
}


void AIStorySeabass::process()
{
	determineState();
	...
	TileFloodfill buildings(_bible, _board);
	buildings.include({_citytype, _outposttype, _farmtype, _barrackstype});
	buildings.exclude({_player});
	buildings.excludeOccupied();
	buildings.execute();
	...
	if (_turnNumber == 1) // only shoot the militia bottom right to show gunner power. Bit strangely coded as I didn't bother to find the easy solution and just copy-pasted.
	{
	...
	} else if (_turnNumber == 7) // Start moving Gunners and riflemen towards enemy targets
	{
	for (Ground& gunner : _myGunners)
	{
	...
	} else if (_turnNumber == 8) // Start moving Militia towards enemy buildings
	{
	for (Ground& militia : _myMilitia)
	{
		...
		if (cityOccupied(destination)) continue; // If this line does what I think it does, it might have to be removed?
		if (!buildings.reached(destination)) continue;
		if (buildings.steps(destination) == 0) continue;
		std::vector<Move> moves;
		Move current;
		while ((current = buildings.step(destination)) != Move::X)
		{
			moves.emplace_back(current);
			destination = destination + current;
		}
	...
	} else if (_turnNumber > 8)
	{
		// Create militia and riflemen if enough power in cities and barracks. 
		// Run them all towards the enemy
