...
AIActingAlligator::Tile AIActingAlligator::makeTile(Cell index) const
{
	Tile tile;
...
AIActingAlligator::Ground AIActingAlligator::makeGround(Cell index) const
{
	Ground ground;
...
AIActingAlligator::Air AIActingAlligator::makeAir(Cell index) const
{
	Air air;
...
bool AIActingAlligator::cityOccupied(Cell index) const
{
...
int8_t AIActingAlligator::cityNiceness(Cell index)
{
	int niceness = 0;
...
int AIActingAlligator::expectedSoil(Cell index)
{
	int expected = 0;
	for (Cell at : _board.area(index, 1, 2))
	{
...
AIActingAlligator::AIActingAlligator(const Player& player, const Difficulty& difficulty,
...
void AIActingAlligator::determineState()
{
	// clear our vectors
	_options.clear();
	_myCities.clear();
...
std::string AIActingAlligator::ainame() const
{
	return "ActingAlligator";
}

std::string AIActingAlligator::authors() const
{
	return "Tom van den Bosch";
}

std::string AIActingAlligator::displayname() const
{
	return "Big evil guy";
}

// this is the part where I start shuffling things around and changing things
void AIActingAlligator::process()
{
	determineState();
	...
	// Gunners move towards enemy city.
	...
	for (Ground& enemyUnit : _enemyGround)
	{
		...
		for (auto& myUnit : surrounding)
		{
			if (surrounding.size() > 0)
			{
				...
				else if (_board.ground(attacker).type == _gunnertype)
				{
					Order order(Order::Type::BOMBARD, myUnit.descriptor,
						Descriptor::cell(enemyUnit.descriptor.position));
					_options.emplace_back(Option{ order, 100 });
				}
			}
