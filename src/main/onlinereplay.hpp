...
class OnlineReplay : public Game, private ClientHandler
{
public:
	OnlineReplay(GameOwner& owner, Settings& settings, Client& client,
		const Role& role,
		const std::string& rulesetname,
		uint32_t planningTime);
	OnlineReplay(const OnlineReplay&) = delete;
	OnlineReplay(OnlineReplay&&) = delete;
	OnlineReplay& operator=(const OnlineReplay&) = delete;
	OnlineReplay& operator=(OnlineReplay&&) = delete;
	virtual ~OnlineReplay() = default;
