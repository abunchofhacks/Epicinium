...
class ClickAndDrag : public InterfaceElement, protected Animator
{
public:
	ClickAndDrag(bool movable = false);
	ClickAndDrag(bool movable, std::unique_ptr<InterfaceElement> shadow);

private:
	std::unique_ptr<InterfaceElement> _shadow;
	...
	bool _moving;
	Pixel _from;
	Pixel _contentTopleft;

	void startMovingFrom(const Pixel& start);
