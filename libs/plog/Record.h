...
#if (PLOG_ENABLE_WCHAR_INPUT && PLOG_FORCE_UTF8_MESSAGE)
        inline void operator<<(std::ostringstream& stream, const wchar_t* data)
        {
            data = data ? data : L"(null)";
...
#   endif
        ...
        inline void operator<<(std::ostringstream& stream, const std::wstring& data)
        {
            plog::detail::operator<<(stream, data.c_str());
        }
#endif
